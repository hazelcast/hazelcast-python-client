...
class DatabaseAPI20Test(unittest.TestCase):
    ...
    def test_paramstyle(self):
        try:
            ...
            # Must be a valid value
            _failUnless(self, paramstyle in ("qmark", "numeric", "named", "format", "pyformat"))
        except AttributeError:
    ...
    def test_cursor_isolation(self):
        ...
        try:
            ...
            cur1.execute(
                "%s into %sbooze values ('Victoria Bitter')" % (self.insert, self.table_prefix)
            )
    ...
    def test_description(self):
        ...
        try:
            ...
            self.assertEqual(
                len(cur.description[0]), 7, "cursor.description[x] tuples must have 7 elements"
            )
            self.assertEqual(
    ...
    def test_executemany(self):
        ...
        try:
            ...
            if self.driver.paramstyle == "qmark":
                cur.executemany(
                    "%s into %sbooze values (?)" % (self.insert, self.table_prefix), largs
                )
            elif self.driver.paramstyle == "numeric":
                cur.executemany(
                    "%s into %sbooze values (:1)" % (self.insert, self.table_prefix), largs
                )
            elif self.driver.paramstyle == "named":
                cur.executemany(
                    "%s into %sbooze values (:beer)" % (self.insert, self.table_prefix), margs
                )
            elif self.driver.paramstyle == "format":
                cur.executemany(
                    "%s into %sbooze values (%%s)" % (self.insert, self.table_prefix), largs
                )
            elif self.driver.paramstyle == "pyformat":
                cur.executemany(
                    "%s into %sbooze values (%%(beer)s)" % (self.insert, self.table_prefix), margs
                )
            else:
    ...
    def test_fetchone(self):
        ...
        try:
            ...
            self.assertEqual(
                cur.fetchone(), None, "cursor.fetchone should return None if no more rows available"
            )
            _failUnless(self, cur.rowcount in (-1, 1))
        finally:
            con.close()
    ...
    def test_fetchmany(self):
        ...
        try:
            ...
            for i in range(0, 6):
                self.assertEqual(
                    rows[i], self.samples[i], "incorrect data retrieved by cursor.fetchmany"
                )
    ...
    def test_fetchall(self):
        ...
        try:
            ...
            self.assertEqual(
                len(rows), len(self.samples), "cursor.fetchall did not retrieve all rows"
            )
            rows = [r[0] for r in rows]
            rows.sort()
    ...
    def test_arraysize(self):
        ...
        try:
            cur = con.cursor()
            _failUnless(self, hasattr(cur, "arraysize"), "cursor.arraysize must be defined")
        finally:
            con.close()
    ...
    def test_STRING(self):
        _failUnless(self, hasattr(self.driver, "STRING"), "module.STRING must be defined")

    def test_BINARY(self):
        _failUnless(self, hasattr(self.driver, "BINARY"), "module.BINARY must be defined.")

    def test_NUMBER(self):
        _failUnless(self, hasattr(self.driver, "NUMBER"), "module.NUMBER must be defined.")

    def test_DATETIME(self):
        _failUnless(self, hasattr(self.driver, "DATETIME"), "module.DATETIME must be defined.")

    def test_ROWID(self):
        _failUnless(self, hasattr(self.driver, "ROWID"), "module.ROWID must be defined.")
