...
class CompactTestBase(unittest.IsolatedAsyncioTestCase, HazelcastTestCase):
    rc = None
    cluster = None
    member = None
...
class CompactTest(CompactTestBase):
    async def test_write_then_read_with_all_fields(self):
        serializer = SomeFieldsSerializer.from_kinds(FIELD_KINDS)
        await self._write_then_read(FIELD_KINDS, REFERENCE_OBJECTS, serializer)

    async def test_write_then_read_with_no_fields(self):
        serializer = SomeFieldsSerializer.from_kinds([])
        await self._write_then_read([], {}, serializer)

    async def test_write_then_read_with_just_var_sized_fields(self):
        serializer = SomeFieldsSerializer.from_kinds(VAR_SIZED_FIELD_KINDS)
        await self._write_then_read(VAR_SIZED_FIELD_KINDS, REFERENCE_OBJECTS, serializer)

    async def test_write_then_read_with_just_fix_sized_fields(self):
        serializer = SomeFieldsSerializer.from_kinds(FIX_SIZED_FIELD_KINDS)
        await self._write_then_read(FIX_SIZED_FIELD_KINDS, REFERENCE_OBJECTS, serializer)

    async def test_write_then_read_object_with_different_position_readers(self):
        params = [
            ("uint8_reader", 1),
            ("uint16_reader", 20),
            ("int32_reader", 42),
        ]
        for name, array_item_count in params:
            with self.subTest(name, array_item_count=array_item_count):
                reference_objects = {
                    FieldKind.ARRAY_OF_STRING: [
                        "x" * (i * 100) for i in range(1, array_item_count)
                    ],
                    FieldKind.INT32: 32,
                ...
                await self._write_then_read(
                    list(reference_objects.keys()), reference_objects, serializer
                )

    async def test_write_then_read_boolean_array(self):
        params = [
            ("0", 0),
            ("1", 1),
            ("8", 8),
        ...
        for name, item_count in params:
            with self.subTest(name, item_count=item_count):
                reference_objects = {
    ...
    async def test_write_and_read_with_multiple_boolean_fields(self):
        params = [
            ("0", 0),
            ("1", 1),
            ("8", 8),
        ...
        for name, field_count in params:
            with self.subTest(name, field_count=field_count):
                ...
                await self._write_then_read0(all_fields, Serializer(list(all_fields.keys())))

    async def test_write_then_read(self):
        params = [(field_kind.name, field_kind) for field_kind in FIELD_KINDS]
        if not params:
            self.skipTest("empty")
        for name, field_kind in params:
            with self.subTest(name, field_kind=field_kind):
                field_name = field_kind.name.lower()
                m = await self._put_entry(
    ...
    async def test_write_none_then_read(self):
        params = [(field_kind.name, field_kind) for field_kind in VAR_SIZED_FIELD_KINDS]
        if not params:
            self.skipTest("empty")
        for name, field_kind in params:
            with self.subTest(name, field_kind=field_kind):
    ...
    async def test_write_array_with_none_items_then_read(self):
        params = [
    ...
    async def test_read_when_field_does_not_exist(self):
        params = [(field_kind.name, field_kind) for field_kind in FIELD_KINDS]
        if not params:
            self.skipTest("empty")
        for name, field_kind in params:
            with self.subTest(name, field_kind=field_kind):
                ...
                evolved_m = await client.get_map(map_name)
                with self.assertRaisesRegex(HazelcastSerializationError, "No field with the name"):
                    await evolved_m.get("key")

    async def test_read_with_type_mismatch(self):
        params = [(field_kind.name, field_kind) for field_kind in FIELD_KINDS]
        if not params:
            self.skipTest("empty")
        for name, field_kind in params:
            with self.subTest(name, field_kind=field_kind):
                ...
                m = await client.get_map(map_name)
                with self.assertRaisesRegex(HazelcastSerializationError, "Mismatched field types"):
                    await m.get("key")

    async def test_write_then_read_as_nullable(self):
        params = [
        ...
        if not params:
            self.skipTest("empty")
        for name, field_kind, nullable_field_kind in params:
            with self.subTest(name, field_kind=field_kind, nullable_field_kind=nullable_field_kind):
                map_name = random_string()
                field_name = field_kind.name.lower()
                await self._put_entry(
                    map_name=map_name,
    ...
    async def test_write_as_nullable_then_read(self):
        params = [
            (field_kind.name, field_kind, nullable_field_kind)
            for field_kind, nullable_field_kind in itertools.chain(
                FIX_SIZED_TO_NULLABLE.items(),
    ...
    async def test_write_nullable_fix_sized_as_none_then_read_as_fix_sized(self):
        params = [
            (field_kind.name, field_kind, nullable_field_kind)
        ...
        for name, field_kind, nullable_field_kind in params:
            with self.subTest(name, field_kind=field_kind, nullable_field_kind=nullable_field_kind):
                ...
                with self.assertRaisesRegex(
                    HazelcastSerializationError, "A 'None' value cannot be read"
                ):
                    await m.get("key")

    async def test_write_nullable_fix_sized_array_with_none_item_then_read_as_fix_sized_array(self):
        params = [
            (field_kind.name, field_kind, nullable_field_kind)
        ...
        for name, field_kind, nullable_field_kind in params:
            with self.subTest(name, field_kind=field_kind, nullable_field_kind=nullable_field_kind):
                ...
                m = await client.get_map(map_name)
                with self.assertRaisesRegex(
                    HazelcastSerializationError, "A `None` item cannot be read"
                ):
                    await m.get("key")

    async def test_write_then_read_with_default_value(self):
        params = [(field_kind.name, field_kind) for field_kind in FIELD_KINDS]
        if not params:
            self.skipTest("empty")
        for name, field_kind in params:
    ...
    async def test_write_then_read_with_default_value_when_field_name_does_not_match(self):
        params = [(field_kind.name, field_kind) for field_kind in FIELD_KINDS]
        if not params:
            self.skipTest("empty")
        for name, field_kind in params:
    ...
    async def test_write_then_read_with_default_value_when_field_type_does_not_match(self):
        params = [(field_kind.name, field_kind) for field_kind in FIELD_KINDS]
        if not params:
            self.skipTest("empty")
        for name, field_kind in params:
    ...
    async def _put_entry(
        ...
        *,
        ...
        m = await client.get_map(map_name)
        await m.put("key", SomeFields(**{field_name: value_to_put}))
        return m
    ...
    async def _write_then_read(
        ...
        await self._write_then_read0(fields, serializer)

    async def _write_then_read0(
        ...
        m = await client.get_map(random_string())
        await m.put("key", SomeFields(**fields))
        obj = await m.get("key")
...
class CompactSchemaEvolutionTest(CompactTestBase):
    async def test_adding_a_fix_sized_field(self):
        await self._verify_adding_a_field(
            ("int32", 42),
            ("string", "42"),
    ...
    async def test_removing_a_fix_sized_field(self):
        await self._verify_removing_a_field(
            ("int64", 1234),
            ("string", "hey"),
    ...
    async def test_adding_a_var_sized_field(self):
        await self._verify_adding_a_field(
            ("int32", 42),
            ("string", "42"),
    ...
    async def test_removing_a_var_sized_field(self):
        await self._verify_removing_a_field(
            ("int64", 1234),
            ("string", "hey"),
    ...
    async def _create_client(self, serializer: CompactSerializer):
    ...
    async def _verify_adding_a_field(
        self,
        *existing_fields: typing.Tuple[str, typing.Any],
        ...
        v1_client = await self._create_client(v1_serializer)
        v1_map = await v1_client.get_map(map_name)
        v1_fields = {name: value for name, value in existing_fields}
        await v1_map.put("key1", SomeFields(**v1_fields))
        ...
        v2_client = await self._create_client(v2_serializer)
        v2_map = await v2_client.get_map(map_name)
        v2_fields = copy.deepcopy(v1_fields)
        v2_fields[new_field_name] = new_field_value
        await v2_map.put("key2", SomeFields(**v2_fields))
        ...
        careful_client_v2 = await self._create_client(careful_v2_serializer)
        careful_v2_map = await careful_client_v2.get_map(map_name)

        # Old client can read data written by the new client
        v1_obj = await v1_map.get("key2")
        ...
        with self.assertRaisesRegex(HazelcastSerializationError, "No field with the name"):
            await v2_map.get("key1")

        # However, if it has default value, everything should work

        careful_v2_obj = await careful_v2_map.get("key1")
    ...
    async def _verify_removing_a_field(
        self,
        *existing_fields: typing.Tuple[str, typing.Any],
        ...
        careful_client_v1 = await self._create_client(careful_v1_serializer)
        careful_v1_map = await careful_client_v1.get_map(map_name)

        # Old client cannot read data written by the new client, since
        # there is no such field on the new data

        with self.assertRaisesRegex(HazelcastSerializationError, "No field with the name"):
            await v1_map.get("key2")

        # However, if it has default value, everything should work
        v1_obj = await careful_v1_map.get("key2")
        for name in v1_fields:
            self.assertEqual(
...
class CompactOnClusterRestartTest(CompactTestBase):
    async def test_cluster_restart(self):
        client = await self.create_client(
            {
                "cluster_name": self.cluster.id,
        ...
        m = await client.get_map(random_string())
        await m.put(1, SomeFields(int32=42))
        ...
        self.assertEqual(1, len(await m.values(sql("int32 == 42"))))
...
class CompactWithListenerTest(CompactTestBase):
    async def test_map_listener(self):
        config = {
            "cluster_name": self.cluster.id,
            "compact_serializers": [SomeFieldsSerializer([FieldDefinition(name="int32")])],
        }
        ...
        await m.add_entry_listener(include_value=True, added_func=listener)
        ...
        other_client = await self.create_client(config)
        other_client_map = await other_client.get_map(map_name)
        await other_client_map.put(1, SomeFields(int32=42))
        await self.assertTrueEventually(lambda: self.assertEqual(1, counter.get()))
