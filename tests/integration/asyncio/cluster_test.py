...
class ClusterTest(unittest.IsolatedAsyncioTestCase, HazelcastTestCase):
    rc = None
    ...
    async def test_initial_membership_listener(self):
        events = []
    ...
    async def test_member_added(self):
        events = []
    ...
    async def test_member_removed(self):
        events = []
    ...
    async def test_exception_in_membership_listener(self):
        def listener(_):
    ...
    async def test_cluster_service_get_members(self):
        self.cluster.start_member()
        config = self.create_config()
        client = await self.create_client(config)
        self.assertEqual(1, len(client.cluster_service.get_members()))

    async def test_cluster_service_get_members_with_selector(self):
        member = self.cluster.start_member()
        config = self.create_config()
        client = await self.create_client(config)
        self.assertEqual(
...
class LoadBalancersWithRealClusterTest(unittest.IsolatedAsyncioTestCase, HazelcastTestCase):
    @classmethod
    def setUpClass(cls):
    ...
    async def test_random_load_balancer(self):
        client = await HazelcastClient.create_and_start(
            cluster_name=self.cluster.id, load_balancer=RandomLB()
        )
    ...
    async def test_round_robin_load_balancer(self):
        client = await HazelcastClient.create_and_start(
            cluster_name=self.cluster.id, load_balancer=RoundRobinLB()
        )
        self.assertTrue(client.lifecycle_service.is_running())
        lb = client._load_balancer
...
class HotRestartEventTest(unittest.IsolatedAsyncioTestCase, HazelcastTestCase):
    @classmethod
    def setUpClass(cls):
