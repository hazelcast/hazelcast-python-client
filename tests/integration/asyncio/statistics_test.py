...
class StatisticsTest(unittest.IsolatedAsyncioTestCase, HazelcastTestCase):
    ...
    async def test_statistics_disabled_by_default(self):
        client = await HazelcastClient.create_and_start(
            cluster_name=self.cluster.id, cluster_connect_timeout=30.0
        )
        await asyncio.sleep(2 * self.DEFAULT_STATS_PERIOD)
    ...
    async def test_statistics_enabled(self):
        client = await HazelcastClient.create_and_start(
            cluster_name=self.cluster.id, cluster_connect_timeout=30.0, statistics_enabled=True
        )
        client_uuid = client._connection_manager.client_uuid
        await asyncio.sleep(2 * self.DEFAULT_STATS_PERIOD)
        await self.wait_for_statistics_collection(client_uuid)
        await client.shutdown()

    async def test_statistics_period(self):
        client = await HazelcastClient.create_and_start(
        ...
        client_uuid = client._connection_manager.client_uuid
        await asyncio.sleep(2 * self.STATS_PERIOD)
        response1 = await self.wait_for_statistics_collection(client_uuid)
        await asyncio.sleep(2 * self.STATS_PERIOD)
        response2 = await self.wait_for_statistics_collection(client_uuid)
        self.assertNotEqual(response1, response2)
        await client.shutdown()

    async def test_statistics_content(self):
        ...
        client_uuid = client._connection_manager.client_uuid
        await client.get_map(map_name)
        await asyncio.sleep(2 * self.STATS_PERIOD)
        response = await self.wait_for_statistics_collection(client_uuid)
    ...
    async def test_near_cache_stats(self):
        map_name = random_string()
        client = await HazelcastClient.create_and_start(
            cluster_name=self.cluster.id,
            cluster_connect_timeout=30.0,
        ...
        client_uuid = client._connection_manager.client_uuid
        test_map = await client.get_map(map_name)
        await asyncio.sleep(2 * self.STATS_PERIOD)
        response = await self.wait_for_statistics_collection(client_uuid)
        result = response.result.decode("utf-8")
        ...
        await test_map.put(1, 2)  # invalidation request
        await test_map.get(1)  # cache miss
        await test_map.get(1)  # cache hit
        await test_map.put(1, 3)  # invalidation + invalidation request
        await test_map.get(1)  # cache miss
        await asyncio.sleep(2 * self.STATS_PERIOD)
        response = await self.wait_for_statistics_collection(client_uuid)
        result = response.result.decode("utf-8")
    ...
    async def test_metrics_blob(self):
        ...
        await client.get_map(map_name)
        await asyncio.sleep(2 * self.STATS_PERIOD)
        response = await self.wait_for_statistics_collection(client_uuid, get_metric_blob=True)
    ...
    async def wait_for_statistics_collection(self, client_uuid, timeout=30, get_metric_blob=False):
