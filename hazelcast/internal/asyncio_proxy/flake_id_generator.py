...
class FlakeIdGenerator(Proxy):
    ...
    Warning:
        Asyncio client FlakeIdGenerator proxy is not thread-safe, do not
    ...
    async def new_id(self) -> int:
        ...
        """
        return await self._auto_batcher.new_id()

    def _new_id_batch(self, batch_size) -> asyncio.Future:
        def handler(message):
...
class _AutoBatcher:
    ...
    async def new_id(self) -> int:
        while True:
            ...
            # In asyncio there is no preemption between non-await statements,
            # so block cannot change here. The check mirrors the threading
            # version for clarity.
...
async def create_flake_id_generator_proxy(service_name, name, context):
    return FlakeIdGenerator(service_name, name, context)
