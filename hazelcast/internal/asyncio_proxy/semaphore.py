...
from hazelcast.util import check_not_negative, check_true, to_millis
...
class Semaphore(BaseCPProxy):
    ...
    async def init(self, permits: int) -> bool:
    ...
    async def drain_permits(self) -> int:
    ...
    async def reduce_permits(self, reduction: int) -> None:
        ...
        return await self._do_change_permits(-reduction)

    async def increase_permits(self, increase: int) -> None:
        ...
        return await self._do_change_permits(increase)
    ...
    async def try_acquire(self, permits: int = 1, timeout: float = 0) -> bool:
    ...
    async def _do_change_permits(self, permits):
...
class SessionAwareSemaphore(Semaphore, SessionAwareCPProxy):
    ...
    async def _do_acquire(self, current_thread_id, invocation_uuid, permits):
        async def do_acquire_once(session_id):
            try:
                ...
                self._invalidate_session(session_id)
                return await self._do_acquire(current_thread_id, invocation_uuid, permits)
        ...
        session_id = await self._acquire_session(permits)
        await do_acquire_once(session_id)

    async def _do_drain(self, current_thread_id, invocation_uuid):
        async def do_drain_once(session_id):
            try:
                count = await self._request_drain(session_id, current_thread_id, invocation_uuid)
        ...
        session_id = await self._acquire_session(_DRAIN_SESSION_ACQ_COUNT)
    ...
    async def _do_change_permits(self, delta):
        ...
        await do_change_permits_once(session_id)

    async def _do_try_acquire(self, current_thread_id, invocation_uuid, permits, timeout):
        start = time.time()

        async def do_try_acquire_once(session_id):
            try:
                acquired = await self._request_acquire(
        ...
        session_id = await self._acquire_session(permits)
        return await do_try_acquire_once(session_id)

    def _new_illegal_state_error(self, cause=None):
        return IllegalStateError('Semaphore["%s"] has no valid session!' % self._object_name, cause)

    async def _request_acquire(
        self, session_id, current_thread_id, invocation_uuid, permits, timeout
    ):
    ...
    async def _request_drain(self, session_id, current_thread_id, invocation_uuid):
    ...
    async def _request_change(self, session_id, current_thread_id, invocation_uuid, delta):
    ...
    async def _request_release(self, session_id, current_thread_id, invocation_uuid, permits):
...
class SessionlessSemaphore(Semaphore):
    ...
    async def acquire(self, permits=1):
        check_true(permits > 0, "Permits must be positive")
        tid = await self._get_thread_id()
        await self._do_try_acquire(tid, permits, -1)

    async def drain_permits(self):
        tid = await self._get_thread_id()
        return await self._do_drain_permits(tid)

    async def release(self, permits=1):
        check_true(permits > 0, "Permits must be positive")
        invocation_uuid = uuid.uuid4()
        tid = await self._get_thread_id()
        return await self._request_release(tid, invocation_uuid, permits)

    async def try_acquire(self, permits=1, timeout=0):
        check_true(permits > 0, "Permits must be positive")
        timeout = max(0.0, timeout)
        tid = await self._get_thread_id()
        return await self._do_try_acquire(tid, permits, timeout)

    async def _do_try_acquire(self, global_thread_id, permits, timeout):
        invocation_uuid = uuid.uuid4()
        try:
            return await self._request_acquire(global_thread_id, invocation_uuid, permits, timeout)
        except WaitKeyCancelledError:
            raise IllegalStateError(
    ...
    async def _do_drain_permits(self, global_thread_id):
        invocation_uuid = uuid.uuid4()
        codec = semaphore_drain_codec
        request = codec.encode_request(
    ...
    async def _do_change_permits(self, permits):
        invocation_uuid = uuid.uuid4()
        tid = await self._get_thread_id()
        return await self._request_change(tid, invocation_uuid, permits)

    async def _request_acquire(self, global_thread_id, invocation_uuid, permits, timeout):
        codec = semaphore_acquire_codec
        if timeout >= 0:
            timeout = to_millis(timeout)
    ...
    async def _request_change(self, global_thread_id, invocation_uuid, permits):
        codec = semaphore_change_codec
        request = codec.encode_request(
            self._group_id,
            self._object_name,
    ...
    async def _request_release(self, global_thread_id, invocation_uuid, permits):
        codec = semaphore_release_codec
        request = codec.encode_request(
            self._group_id,
            self._object_name,
    ...
    async def _get_thread_id(self):
        return await self._session_manager.get_or_create_unique_thread_id(self._group_id)
