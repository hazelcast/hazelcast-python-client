...
def handle(msg, handle_distributed_object_event=None):
    ...
    if (
        message_type == _EVENT_DISTRIBUTED_OBJECT_MESSAGE_TYPE
        and handle_distributed_object_event is not None
    ):
        initial_frame = msg.next_frame()
        source = FixSizedTypesCodec.decode_uuid(
            initial_frame.buf, _EVENT_DISTRIBUTED_OBJECT_SOURCE_OFFSET
        )
