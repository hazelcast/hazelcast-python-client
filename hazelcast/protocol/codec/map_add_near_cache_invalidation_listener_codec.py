...
_EVENT_I_MAP_INVALIDATION_PARTITION_UUID_OFFSET = (
    _EVENT_I_MAP_INVALIDATION_SOURCE_UUID_OFFSET + UUID_SIZE_IN_BYTES
)
_EVENT_I_MAP_INVALIDATION_SEQUENCE_OFFSET = (
    _EVENT_I_MAP_INVALIDATION_PARTITION_UUID_OFFSET + UUID_SIZE_IN_BYTES
)
...
def handle(msg, handle_i_map_invalidation_event=None, handle_i_map_batch_invalidation_event=None):
    ...
    if (
        message_type == _EVENT_I_MAP_INVALIDATION_MESSAGE_TYPE
        and handle_i_map_invalidation_event is not None
    ):
        initial_frame = msg.next_frame()
        ...
        return
    if (
        message_type == _EVENT_I_MAP_BATCH_INVALIDATION_MESSAGE_TYPE
        and handle_i_map_batch_invalidation_event is not None
    ):
        msg.next_frame()
