...
REQUEST_TYPE = REPLICATEDMAP_ADDENTRYLISTENERWITHPREDICATE
RESPONSE_TYPE = 104
RETRYABLE = False


def calculate_size(name, predicate, local_only):
...
def encode_request(name, predicate, local_only):
    ...
    client_message = ClientMessage(payload_size=calculate_size(name, predicate, local_only))
