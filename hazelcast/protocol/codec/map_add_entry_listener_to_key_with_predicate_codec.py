...
REQUEST_TYPE = MAP_ADDENTRYLISTENERTOKEYWITHPREDICATE
RESPONSE_TYPE = 104
RETRYABLE = False


def calculate_size(name, key, predicate, include_value, listener_flags, local_only):
...
def encode_request(name, key, predicate, include_value, listener_flags, local_only):
    ...
    client_message = ClientMessage(payload_size=calculate_size(name, key, predicate, include_value, listener_flags, local_only))
