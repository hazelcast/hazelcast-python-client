...
REQUEST_TYPE = MAPREDUCE_FORMULTIMAP
RESPONSE_TYPE = 117
RETRYABLE = False


def calculate_size(name, job_id, predicate, mapper, combiner_factory, reducer_factory, multi_map_name, chunk_size, keys, topology_changed_strategy):
    ...
    data_size += calculate_size_str(multi_map_name)
...
def encode_request(name, job_id, predicate, mapper, combiner_factory, reducer_factory, multi_map_name, chunk_size, keys, topology_changed_strategy):
    """ Encode request into client_message"""
    client_message = ClientMessage(payload_size=calculate_size(name, job_id, predicate, mapper, combiner_factory, reducer_factory, multi_map_name, chunk_size, keys, topology_changed_strategy))
    ...
    client_message.append_str(multi_map_name)
