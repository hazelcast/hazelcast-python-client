...
_INITIAL_FRAME_SIZE = (
    _LITE_MEMBER_ENCODE_OFFSET + BOOLEAN_SIZE_IN_BYTES - 2 * SIZE_OF_FRAME_LENGTH_AND_FLAGS
)
...
class MemberInfoCodec(object):
    ...
    def encode(buf, member_info, is_final=False):
        ...
        FixSizedTypesCodec.encode_boolean(
            initial_frame_buf, _LITE_MEMBER_ENCODE_OFFSET, member_info.lite_member
        )
        ...
        MapCodec.encode(
            buf, member_info.address_map, EndpointQualifierCodec.encode, AddressCodec.encode
        )
        if is_final:
    ...
    def decode(msg):
        ...
        return MemberInfo(
            address, uuid, attributes, lite_member, version, is_address_map_exists, address_map
        )
