...
_INITIAL_FRAME_SIZE = (
    _ITERATION_TYPE_ID_ENCODE_OFFSET + BYTE_SIZE_IN_BYTES - 2 * SIZE_OF_FRAME_LENGTH_AND_FLAGS
)
...
class PagingPredicateHolderCodec(object):
    ...
    def encode(buf, paging_predicate_holder, is_final=False):
        ...
        FixSizedTypesCodec.encode_int(
            initial_frame_buf, _PAGE_SIZE_ENCODE_OFFSET, paging_predicate_holder.page_size
        )
        FixSizedTypesCodec.encode_int(
            initial_frame_buf, _PAGE_ENCODE_OFFSET, paging_predicate_holder.page
        )
        FixSizedTypesCodec.encode_byte(
            initial_frame_buf,
            _ITERATION_TYPE_ID_ENCODE_OFFSET,
            paging_predicate_holder.iteration_type_id,
        )
    ...
    def decode(msg):
        ...
        iteration_type_id = FixSizedTypesCodec.decode_byte(
            initial_frame.buf, _ITERATION_TYPE_ID_DECODE_OFFSET
        )
