...
_INITIAL_FRAME_SIZE = (
    _ERROR_CODE_ENCODE_OFFSET + INT_SIZE_IN_BYTES - 2 * SIZE_OF_FRAME_LENGTH_AND_FLAGS
)
...
class ErrorHolderCodec(object):
    ...
    def encode(buf, error_holder, is_final=False):
        ...
        FixSizedTypesCodec.encode_int(
            initial_frame_buf, _ERROR_CODE_ENCODE_OFFSET, error_holder.error_code
        )
        ...
        ListMultiFrameCodec.encode(
            buf, error_holder.stack_trace_elements, StackTraceElementCodec.encode
        )
        if is_final:
