...
class AtomicReference(PartitionSpecificProxy):
    def compare_and_set(self, expected, updated):
        return self._encode_invoke_on_partition(atomic_reference_compare_and_set_codec,
                                                expected=self._to_data(expected), updated=self._to_data(updated))

    def get(self):
        return self._encode_invoke_on_partition(atomic_reference_get_codec)

    def set(self, new_value):
        return self._encode_invoke_on_partition(atomic_reference_set_codec,
                                                new_value=self._to_data(new_value))

    def get_and_set(self, new_value):
        return self._encode_invoke_on_partition(atomic_reference_get_and_set_codec,
                                                new_value=self._to_data(new_value))

    def set_and_get(self, new_value):
        return self._encode_invoke_on_partition(atomic_reference_set_and_get_codec,
                                                new_value=self._to_data(new_value))

    def is_null(self):
        return self._encode_invoke_on_partition(atomic_reference_is_null_codec)

    def clear(self):
        return self._encode_invoke_on_partition(atomic_reference_clear_codec)

    def contains(self, expected):
        return self._encode_invoke_on_partition(atomic_reference_contains_codec,
