...
class Map(Proxy):
    ...
    This map can work both async(non-blocking) or sync(blocking). Blocking calls return the value of the call and blocks
    the execution until return value is calculated. However, async calls return Future and do not block execution.
    Result of the Future can be used whenever ready. A Future's result can be obtained with blocking the execution by
    ...
    def add_entry_listener(self, include_value=False, key=None, predicate=None, added_func=None, removed_func=None, updated_func=None,
        ...
        """
        Adds an continuous entry listener for this map. Listener will get notified for map events filtered with given
        parameters.
        ...
        :param removed_func: Function to be called called when an entry is removed_func from map (optional).
        :param updated_func: Function to be called called when an entry is updated (optional).
        :param evicted_func: Function to be called called when an entry is evicted_func from map (optional).
        :param evict_all_func: Function to be called called when entries are evicted_func from map (optional).
        :param clear_all_func: Function to be called called when entries are cleared from map (optional).
        :param merged_func: Function to be called called when WAN replicated entry is merged_func (optional).
        :param expired_func: Function to be called called when an entry's live time is expired (optional).
        :return: (String), a registration id which is used as a key to remove the listener.

        See :ref:`Predicate` for more info about predicates.
        """
    ...
    def add_index(self, attribute, ordered=False):
        ...
        Example:
            ...
            If you are querying your values mostly based on age and active then you should consider indexing these
            fields.
        ...
        :param attribute: (String), index attribute of the value.
    ...
    def add_interceptor(self, interceptor):
        ...
        :return: (String),id of registered interceptor.
        """
    ...
    def contains_key(self, key):
        ...
        **Warning:
        This method uses GetHashCode and Equals of binary form of the key, not the actual implementations of GetHashCode
        and Equals defined in key's class.**
    ...
    def delete(self, key):
        ...
        **Warning:
        ...
        with a None oldValue. Also, a listener with predicates will have None values, so only keys can be queried via
        predicates.**
    ...
    def entry_set(self, predicate=None):
        """
        Returns a List clone of the mappings contained in this map. The list is NOT backed by the map, so changes to the
        map are NOT reflected in the list, and vice-versa.

        :param predicate: (Predicate), predicate for the map to filter entries (optional).
        :return: (List), result entry list of the query.
    ...
    def execute_on_entries(self, entry_processor, predicate=None):
        """
        Applies the user defined EntryProcessor to the all entries in the map or entries in the map which satisfies
        the predicate if provided. Returns the results mapped by each key in the map.

        :param entry_processor: (Object), user defined EntryProcessor.
        :param predicate: (Predicate), predicate for filtering the entries (optional).
        :return: (List), list of map entries which includes the keys and the results of the entry process.
    ...
    def execute_on_key(self, key, entry_processor):
        """
        Applies the user defined EntryProcessor to the entry mapped by the key. Returns the the object which is result
        of the process method of EntryProcessor.
    ...
    def execute_on_keys(self, keys, entry_processor):
        ...
        :param keys: (List), collection of the keys for the entries to be processed.
        :param entry_processor: (Object), user defined EntryProcessor.
        :return: (List), list of map entries which includes the keys and the results of the entry process.
        """
        key_list = []
    ...
    def get_all(self, keys):
        ...
        **Warning 2:
        ...
        :param keys: (List), keys to get.
        :return: (List), list of map entries.
        """
    ...
    def get_entry_view(self, key):
        ...
        **Warning 2:
        ...
        See :ref:`EntryView` for more info about EntryView.
        """
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
    ...
    def key_set(self, predicate=None):
        ...
        **Warning:
        ...
        :return: (List), a list of the clone of the keys.
    ...
    def load_all(self, keys=None, replace_existing_values=True):
        """
        Loads all keys into the server side store or load the given keys if provided.

        :param keys: (List), keys of the values entries to load (optional).
    ...
    def lock(self, key, ttl=-1):
        ...
        Locks are re-entrant so if the key is locked N times then it should be unlocked N times before another thread
        can acquire it.
    ...
    def put(self, key, value, ttl=-1):
        ...
        **Warning 2:
        ...
        :return: (Object), previous value associated with key or null if there was no mapping for key.
        """
        check_not_none(key, "key can't be None")
        check_not_none(value, "value can't be None")
        key_data = self._to_data(key)
    ...
    def put_all(self, map):
        """
        Copies all of the mappings from the specified map to this map (optional operation). No atomicity guarantees are
        given. It could be that in case of failure some of the key/value-pairs get written, while others are not.

        :param map: (Map), map which includes mappings to be stored in this map
        """
    ...
    def put_if_absent(self, key, value, ttl=-1):
        """
        If the specified key is not already associated with a value, associate it with the given value. If ttl is
        provided, entry will expire and get evicted after the ttl.
    ...
    def put_transient(self, key, value, ttl=-1):
        """
        Same as put(TKey, TValue, Ttl) but server side defined MapStore will not be called.
    ...
    def remove_entry_listener(self, registration_id):
        ...
        :param registration_id: (String), id of registered listener.
    ...
    def set(self, key, value, ttl=-1):
        ...
        **Warning:
        ...
        :param key: (Object), key of the entry.
        :param value: (Object), value of the entry.
        :param ttl: (int), maximum time in seconds for this entry to stay in the map, 0 means infinite (optional).
        """
        check_not_none(key, "key can't be None")
        check_not_none(value, "value can't be None")
        key_data = self._to_data(key)
    ...
    def try_lock(self, key, ttl=-1, timeout=0):
        ...
        Tries to acquire the lock for the specified key. When the lock is not available,
            ...
            * If timeout is not provided, then the current thread doesn't wait and returns false immediately.
            * If a timeout is provided, then the current thread becomes disabled for thread scheduling purposes and lies
        ...
        :param ttl: (int), time in seconds to wait before releasing the lock.
    ...
    def try_remove(self, key, timeout=0):
        ...
        timeout is provided, operation waits until it is completed or timeout is reached.git
    ...
    def values(self, predicate=None):
        """
        Returns a collection clone of the values contained in this map or values of the entries which are filtered with
        the predicate if provided.

        **Warning:
        The collection is NOT backed by the map, so changes to the map are NOT reflected in the collection, and
        vice-versa.**

        :param predicate: (Predicate), predicate to filter the entries (optional).
        :return: (List), a list of clone of the values contained in this map.
