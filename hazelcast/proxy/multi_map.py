...
class MultiMap(Proxy):
    ...
    def contains_key(self, key):
        ...
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_contains_key_codec, key_data, name=self.name, key=key_data,
                                          thread_id=thread_id())

    def contains_value(self, value):
        check_not_none(value, "value can't be None")
        value_data = self._to_data(value)
        return self._encode_invoke(multi_map_contains_value_codec, name=self.name, value=value_data)
    ...
    def contains_entry(self, key, value):
        ...
        key_data = self._to_data(key)
        value_data = self._to_data(value)
        return self._encode_invoke_on_key(multi_map_contains_entry_codec, key_data, name=self.name, key=key_data,
                                          value=value_data, thread_id=thread_id())

    def clear(self):
        return self._encode_invoke(multi_map_clear_codec, name=self.name)

    def entry_set(self):
        return self._encode_invoke(multi_map_entry_set_codec, name=self.name)

    def get(self, key):
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_get_codec, key_data, name=self.name, key=key_data,
                                          thread_id=thread_id())

    def is_locked(self, key):
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_is_locked_codec, key_data, name=self.name, key=key_data)

    def force_unlock(self, key):
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_force_unlock_codec, key_data, name=self.name, key=key_data)

    def key_set(self):
        return self._encode_invoke(multi_map_key_set_codec, name=self.name)

    def lock(self, key, lease_time=-1):
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_lock_codec, key_data, name=self.name, key=key_data,
    ...
    def remove(self, key, value):
        ...
        key_data = self._to_data(key)
        value_data = self._to_data(value)
        return self._encode_invoke_on_key(multi_map_remove_entry_codec, key_data, name=self.name, key=key_data,
                                          value=value_data, thread_id=thread_id())

    def remove_all(self, key):
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_remove_codec, key_data, name=self.name, key=key_data,
                                          thread_id=thread_id())
    ...
    def put(self, key, value):
        ...
        key_data = self._to_data(key)
        value_data = self._to_data(value)
        return self._encode_invoke_on_key(multi_map_put_codec, key_data, name=self.name, key=key_data, value=value_data,
                                          thread_id=thread_id())
    ...
    def size(self):
        return self._encode_invoke(multi_map_size_codec, name=self.name)

    def value_count(self, key):
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_value_count_codec, key_data, name=self.name, key=key_data,
                                          thread_id=thread_id())

    def values(self):
        return self._encode_invoke(multi_map_values_codec, name=self.name)

    def try_lock(self, key, lease_time=-1, timeout=-1):
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_try_lock_codec, key_data, name=self.name, key=key_data,
    ...
    def unlock(self, key):
        check_not_none(key, "key can't be None")
        key_data = self._to_data(key)
        return self._encode_invoke_on_key(multi_map_unlock_codec, key_data, name=self.name, key=key_data,
                                          thread_id=thread_id())
