...
class MultiMap(Proxy):
    ...
    def add_entry_listener(self, include_value=False, key=None, added_func=None, removed_func=None, clear_all_func=None):
        ...
        if key:
            ...
            return self._register_listener(multi_map_add_entry_listener_to_key_codec.encode_request(
                name=self.name, key=key_data, include_value=include_value, local_only=False),
                lambda r: multi_map_add_entry_listener_to_key_codec.decode_response(r)['response'],
                lambda reg_id: multi_map_remove_entry_listener_codec.encode_request(self.name, reg_id),
                lambda m: multi_map_add_entry_listener_to_key_codec.handle(m, handle_event_entry, self._to_object))
        else:
            return self._register_listener(multi_map_add_entry_listener_codec.encode_request(
                name=self.name, include_value=include_value, local_only=False),
                lambda r: multi_map_add_entry_listener_codec.decode_response(r)['response'],
                lambda reg_id: multi_map_remove_entry_listener_codec.encode_request(self.name, reg_id),
                lambda m: multi_map_add_entry_listener_codec.handle(m, handle_event_entry, self._to_object))
